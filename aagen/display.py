...
class DungeonDisplay:
    ...
    def update_bounds(self, (display_w, display_h)=(None, None)):
        ...
        log.debug("Buffered bounds are {0}".format((x0, y0, x1, y1)))
        ...
        # Center the dungeon in the display as needed, but fudge it a bit
        # to ensure that the origin is a grid intersection.
        # The display uses left-handed coordinates while the map uses
        # right-handed coordinates, so the origin for the display (its
        # top left corner) is the x *minimum* and y *maximum* of the map.
        delta = (display_h / self.display_scale) - map_h
        y = math.floor((y1 + delta/2) / 10) * 10
        delta = (display_w / self.display_scale) - map_w
        x = math.ceil((x0 - delta/2) / 10) * 10

        self.origin = (x, y)

        log.debug("Map display origin is now {0}, scale is {1}"
                  .format(self.origin, self.display_scale))
